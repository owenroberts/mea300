...
//            console.log('anti gravity', antiCounter);
    ...
    if (keyWentDown("space") && !player.isJumping) {
        // play jump sound
    ...
    arrows.overlap(player, function (arrow) {
        ...
        // spawn a rotten
        var x = random(rottenXMin, rottenXMax);
        var y = random(rottenYMin, rottenYMax);
        var rotten = createSprite(x, y);
        rotten.addAnimation('default', rotten_animation);
        rotten.velocity.x = -rottenSpeed;
        rottens.add(rotten);
    });
    ...
    // player hits rotten
    rottens.overlap(player, function (rotten) {
        
        var x = rotten.position.x;
        var y = rotten.position.y;
        var rottenHit = createSprite(x, y);
        rottenHit.addAnimation("default", rotten_hit_animation);
        rottenHit.life = 40;
        rotten.remove();
//        player.livesLeft--;
        removeLife();
    ...
    // remove un gathered rottens
    for (var i = 0; i < rottens.length; i++) {
        if (rottens[i].position.x < -50) {
            rottens[i].remove();
        }
    }
    ...
    for (var i = 0; i < bushes.length; i++) {
        if (bushes[i].position.x < -95) {
            bushes[i].position.x = random(width, width * 2);
        }
    }
